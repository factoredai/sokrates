...
def make_dataset_from_path(fp: str) -> pd.DataFrame:
    """
    Given a path to a Posts.xml file or a folder that contains one, the
    function parses the XML, runs the feature extraction and returns the
    resulting dataframe.
    :param fp:
    :return:
    """
    data = XMLparser(fp)
    data = ManualFeatureExtract.process_df(data)
    return data
